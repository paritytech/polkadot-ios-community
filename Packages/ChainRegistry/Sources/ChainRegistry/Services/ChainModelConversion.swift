...
public protocol ChainModelConversionProtocol {
    func update(
...
public final class ChainModelConverter: ChainModelConversionProtocol {
    public init() {}
    ...
    public func update(
        ...
        let chainAssets = remoteAssets.map { remoteAsset in
            if let localAsset = localAssets?[remoteAsset.assetId] {
                AssetModel(remoteModel: remoteAsset, enabled: localAsset.enabled)
            } else {
                AssetModel(remoteModel: remoteAsset, enabled: true)
            }
        }
    ...
    private func determineSyncMode(basedOn localModel: ChainModel?, remoteModel: RemoteChainModel) -> ChainSyncMode {
        ...
        if let localModel, localModel.syncMode == .disabled {
            return .disabled
        }
