import Foundation
@preconcurrency import Operation_iOS
import SubstrateSdk
import SDKLogger
import EventCenter
...
public final class RuntimeProvider {
    ...
    public var chainId: ChainModel.Id { chain.chainId }
    private(set) var chain: RuntimeProviderChain

    public let snapshotOperationFactory: RuntimeSnapshotFactoryProtocol
...
extension RuntimeProvider: RuntimeProviderProtocol {
    ...
    public func replaceChainData(_ chain: ChainModel) {
        mutex.lock()
...
extension RuntimeProvider: ChainRegistryEventVisiting {
    public func processRuntimeChainTypesSyncCompleted(event: RuntimeChainTypesSyncCompleted) {
    ...
    public func processRuntimeChainMetadataSyncCompleted(event: RuntimeMetadataSyncCompleted) {
        guard event.chainId == chainId else {
            return
        }
    ...
    public func processRuntimeCommonTypesSyncCompleted(event: RuntimeCommonTypesSyncCompleted) {
