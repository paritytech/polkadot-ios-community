...
public enum ConnectionFactoryError: Error {
    case noNodes
    case noApiKeys
    case invalidNode
}
...
extension ConnectionFactory: ConnectionFactoryProtocol {
    ...
    public func updateConnection(_ connection: ChainConnection, chain: ChainNodeConnectable) {
    ...
    public func createOneShotConnection(for chain: ChainNodeConnectable) throws -> OneShotConnection {
    ...
    public func updateOneShotConnection(
        _ connection: OneShotConnection,
        chain: ChainNodeConnectable
    ) {
    ...
    private func nodeUrl(from node: ChainNodeModel) -> ConnectionCreationParams? {
        ...
        guard let url = try? builder.buildBy(closure: { apiKeyType in
            guard let apiKey = apiKeysProvider.getKey(by: apiKeyType) else {
                throw ConnectionFactoryError.noApiKeys
            }
    ...
    private func extractNodeUrls(
        ...
        let filteredNodes = chain.nodes.filter { schemaPredicate.evaluate(with: $0.url) }

        let nodes: [ChainNodeModel] =
            switch chain.nodeSwitchStrategy {
            case .roundRobin:
                filteredNodes.sorted(by: { $0.order < $1.order })
            case .uniform:
                filteredNodes.shuffled()
            }
