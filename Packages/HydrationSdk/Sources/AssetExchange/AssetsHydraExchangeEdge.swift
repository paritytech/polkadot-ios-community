...
class AssetsHydraExchangeEdge {
    ...
    var type: AssetExchangeEdgeType { AssetExchangeReservedType.hydraSwap.rawValue }
    ...
    func beginMetaOperation(
        ...
        let assetIn = try host.chain.chainAssetInterfaceOrError(for: origin.assetId)

        let assetOut = try host.chain.chainAssetInterfaceOrError(for: destination.assetId)
    ...
    func appendToMetaOperation(
        ...
        let newAssetOut = try host.chain.chainAssetInterfaceOrError(for: destination.assetId)
    ...
    func addingWeight(
        ...
        return currentWeight + (suggestedEdgeWeight / HydraExchange.mergingEdgesWeightDivider)
    }
}
