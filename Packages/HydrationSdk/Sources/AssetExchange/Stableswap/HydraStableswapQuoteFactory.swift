...
enum HydraStableswapQuoteFactoryError: Error {
    case unexpectedPoolInfo
    case unexpectedAssetDecimals
}
...
final class HydraStableswapQuoteFactory {
    ...
    private func deriveApiParams(
        ...
        guard
            ...
            throw HydraStableswapQuoteFactoryError.unexpectedPoolInfo
        }
        ...
        let reserves: [HydraStableswapApi.AssetReserveInfo] = try poolInfo.assets.map { asset in
            ...
            guard let decimals = quoteState.getDecimals(for: asset.value) else {
                throw HydraStableswapQuoteFactoryError.unexpectedAssetDecimals
            }
    ...
    private func calculateQuote(
        ...
        if args.assetIn == args.poolAsset {
            ...
            case .sell:
                try calculateSellWhenAssetInPoolAsset(
                    for: apiParams,
                    args: args
                )
            case .buy:
                try calculateBuyWhenAssetInPoolAsset(
                    for: apiParams,
                    args: args
                )
            }
            ...
            switch args.direction {
            case .sell:
                try calculateSellWhenAssetOutPoolAsset(
                    for: apiParams,
                    args: args
                )
            case .buy:
                try calculateBuyWhenAssetOutPoolAsset(
                    for: apiParams,
                    args: args
                )
            }
            ...
            switch args.direction {
            case .sell:
                try calculateSellForNonPoolAssets(
                    for: apiParams,
                    args: args
                )
            case .buy:
                try calculateBuyForNonPoolAssets(
                    for: apiParams,
                    args: args
                )
            }
