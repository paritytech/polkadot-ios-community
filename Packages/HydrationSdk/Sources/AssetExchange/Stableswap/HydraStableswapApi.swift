...
enum HydraStableswapApi {
    ...
    static func calculateOutGivenIn(
        ...
        let result = try HydraStableswapMath.stableswapCalculateOutGivenIn(
            JsonStringify.jsonString(from: params.reserves),
            UInt32(assetIn),
            UInt32(assetOut),
            String(amountIn),
            String(amplification),
            fee,
            JsonStringify.jsonString(from: params.pegs)
        )
    ...
    static func calculateInGivenOut(
        ...
        let result = try HydraStableswapMath.stableswapCalculateInGivenOut(
            JsonStringify.jsonString(from: params.reserves),
            UInt32(assetIn),
            UInt32(assetOut),
            String(amountOut),
    ...
    static func calculateAddOneAsset(
        ...
        let result = try HydraStableswapMath.stableswapCalculateAddOneAsset(
            JsonStringify.jsonString(from: params.reserves),
            String(shareAmount),
            UInt32(asset),
            String(amplification),
    ...
    static func calculateLiquidityOutOneAsset(
        ...
        let result = try HydraStableswapMath.stableswapCalculateLiquidityOutOneAsset(
            JsonStringify.jsonString(from: params.reserves),
            String(shareAmount),
            UInt32(asset),
            String(amplification),
    ...
    static func calculateSharesForAmount(
        ...
        let result = try HydraStableswapMath.stableswapCalculateSharesForAmount(
            JsonStringify.jsonString(from: params.reserves),
            UInt32(asset),
            String(assetAmount),
            String(amplification),
    ...
    static func calculateShares(
        ...
        let result = try HydraStableswapMath.stableswapCalculateShares(
            JsonStringify.jsonString(from: params.reserves),
            JsonStringify.jsonString(from: assets),
            String(amplification),
            String(params.shareAssetIssuance),
            fee,
            JsonStringify.jsonString(from: params.pegs)
