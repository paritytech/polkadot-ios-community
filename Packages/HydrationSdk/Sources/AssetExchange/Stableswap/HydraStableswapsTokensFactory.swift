...
final class HydraStableswapTokensFactory {
    ...
    private func fetchAllLocalPairs(
        ...
        let conversionOperation = ClosureOperation<[ChainAssetId: Set<ChainAssetId>]> {
            ...
            let allLocalAssets = chain.chainAssetsInterface()
            let localRemoteAssets = try allLocalAssets.reduce(
                into: [HydraDx.AssetId: ChainAssetId]()
            ) { accum, chainAsset in
                let pair = try tokenConverter.convertToRemote(
    ...
    private func fetchAllLocalPoolAssets(
        for chain: ChainProtocol,
        tokenConverter: HydrationTokenConverting
...
extension HydraStableswapTokensFactory: HydraPoolTokensFactoryProtocol {
    func availableDirections() -> CompoundOperationWrapper<[ChainAssetId: Set<ChainAssetId>]> {
        fetchAllLocalPairs(for: chain, tokenConverter: tokenConverter)
    }

    func availableDirectionsForAsset(
        _ chainAssetId: ChainAssetId
    ) -> CompoundOperationWrapper<Set<ChainAssetId>> {
        let allPairsWrapper = fetchAllLocalPairs(for: chain, tokenConverter: tokenConverter)
    ...
    func fetchAllLocalPoolAssets() -> CompoundOperationWrapper<Set<ChainAssetId>> {
        fetchAllLocalPoolAssets(for: chain, tokenConverter: tokenConverter)
    }
