...
final class HydraExchangeAtomicOperation {
    typealias Edge = AssetExchangableGraphEdge & AssetsHydraExchangeEdgeProtocol
    ...
    private func createFeeWrapper() -> CompoundOperationWrapper<ExtrinsicFeeProtocol> {
        ...
        let feeWrapper = OperationCombiningService<ExtrinsicFeeProtocol>.compoundNonOptionalWrapper(
            ...
            }, origin: self.host.originDefiner, payingIn: self.operationArgs.feeAsset)
...
extension HydraExchangeAtomicOperation: AssetExchangeAtomicOperationProtocol {
        ...
        let executionWrapper = OperationCombiningService<Balance>.compoundNonOptionalWrapper(
            ...
            let submittionWrapper = self.host.submissionMonitorFactory.submitAndMonitorWrapper(
                ...
                params: ExtrinsicSubmissionParams(
                    ...
                    eventsMatcher: HydraSwapEventsMatcher()
                )
            )
    ...
    func estimateFee(creditingTo accountId: AccountId?) -> CompoundOperationWrapper<AssetExchangeOperationFee> {
        ...
        let destMatchesSigner = accountId == nil || host.selectedAccount.accountId == accountId
        let postTransfer: AssetExchangeOperationFee.PostTransfer = destMatchesSigner ? .free : .notSupported
