...
public final class CrosschainAssetsExchange {
    let host: CrosschainExchangeHostProtocol

    public init(host: CrosschainExchangeHostProtocol) {
        self.host = host
    }
}
...
private extension CrosschainAssetsExchange {
    func createExchange(
        ...
        do {
            let originChain = try host.chainRegistry.getChainInterfaceOrError(for: origin.chainId)
            let originChainAsset = try originChain.chainAssetInterfaceOrError(for: origin.assetId)
            let destinationChain = try host.chainRegistry.getChainInterfaceOrError(for: destination.chainId)
