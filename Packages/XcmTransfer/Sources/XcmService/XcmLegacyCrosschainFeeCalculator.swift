...
private extension XcmLegacyCrosschainFeeCalculator {
    func createModuleResolutionWrapper(
        ...
        switch transferType {
        case .xtokens:
            xTokensQueryFactory.createModuleNameResolutionWrapper(for: runtimeProvider)
        case .xcmpallet,
             .teleport,
             .xcmpalletTransferAssets:
            xcmPalletQueryFactory.createModuleNameResolutionWrapper(for: runtimeProvider)
        case .unknown:
            CompoundOperationWrapper.createWithError(XcmCallTypeError.unknownType)
        }
    }
    ...
    func createStardardFeeEstimationWrapper(
        ...
        do {
            let runtimeProvider = try chainRegistry.getRuntimeCodingServiceOrError(for: chain.chainId)
            ...
            let origin = try originDefiningFactory.extrinsicOriginDefiner(
                from: wallet,
                chain: chain
            )
    ...
    func createOneOfFeeEstimationWrapper(
        ...
        case .standard:
            return createStardardFeeEstimationWrapper(
                chain: chain,
                message: message,
                maxWeight: maxWeight
            )
    ...
    func createDestinationFeeWrapper(
        ...
        createOneOfFeeEstimationWrapper(
            chain: request.destination.chain,
            message: message,
    ...
    func createChainDeliveryFeeWrapper(
        ...
        let optPrice: XcmDeliveryFee.Price? =
            if request.toParachainId != nil {
                deliveryFee.toParachain
            } else {
                deliveryFee.toParent
            }
    ...
    private func createExponentialFactorWrapper(
        ...
        guard let connection = chainRegistry.getRpcConnection(for: chainId) else {
            let error = ChainResourceError.connectionUnavailable
            return CompoundOperationWrapper.createWithError(error)
        }
    ...
    func createExponentialDeliveryFeeWrapper(
        ...
        guard let runtimeProvider = chainRegistry.getRuntimeCodingService(for: request.fromChainId) else {
            let error = ChainResourceError.runtimeMetadaUnavailable
            return CompoundOperationWrapper.createWithError(error)
        }
...
extension XcmLegacyCrosschainFeeCalculator: XcmCrosschainFeeCalculating {
    func crossChainFeeWrapper(
        ...
        do {
            let destinationRuntimeProvider = try chainRegistry.getRuntimeCodingServiceOrError(
                for: request.destination.chain.chainId
            )

            let reserveRuntimeProvider = try chainRegistry.getRuntimeCodingServiceOrError(
                for: request.reserve.chain.chainId
            )
