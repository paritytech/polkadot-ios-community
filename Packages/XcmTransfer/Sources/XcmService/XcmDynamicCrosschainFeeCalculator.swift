...
final class XcmDynamicCrosschainFeeCalculator {
    ...
    init(
        ...
        self.tokenMintingFactory = tokenMintingFactory
...
private extension XcmDynamicCrosschainFeeCalculator {
    func ensureSafeAmountToSend(for request: XcmUnweightedTransferRequest) -> XcmUnweightedTransferRequest {
        let minimumAmount = Self.minimumSendAmount.toSubstrateAmount(
            precision: request.origin.chainAsset.assetInterface.decimalPrecision
        )
    ...
    func ensureSafeAmountToFund(for sendAmount: Balance?, chainAsset: ChainAssetProtocol) -> Balance {
    ...
    func prepareAccountSetupCallsForDryRun(
        ...
        if
            !request.origin.chainAsset.assetInterface.isUtility,
            let nativeAsset = request.originChain.utilityChainAssetInterface() {
