...
extension AssetsExchangeOperationFactory: AssetsExchangeOperationFactoryProtocol {
    ...
    func createFeeWrapper(for args: AssetExchangeFeeArgs) -> CompoundOperationWrapper<AssetExchangeFee> {
        do {
            ...
            let feeWrappers = atomicOperations.enumerated().map { index, atomicOperation in
                if index == atomicOperations.count - 1 {
                    atomicOperation.estimateFee(creditingTo: args.destinationAccountId)
                } else {
                    atomicOperation.estimateFee(creditingTo: nil)
                }
            }
    ...
    func createSingleOperationSubmitWrapper(
        ...
        do {
            ...
            return atomicOperation.submitWrapper(for: swapLimit, creditingTo: accountId)
        } catch {
            return .createWithError(error)
        }
    }
