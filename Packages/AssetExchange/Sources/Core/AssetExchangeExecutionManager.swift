...
final class AssetExchangeExecutionManager {
    ...
    init(
        ...
        syncQueue = DispatchQueue(label: "io.asset.exchange.exec.\(UUID().uuidString)")
    }
    ...
    private func executeSegment(at index: Int, amountIn: Balance) {
        ...
        let receiver = operations.count - 1 == index ? destinationAccountId : nil
        let wrapper = operations[index].executeWrapper(for: swapLimit, creditingTo: receiver)
