...
extension AssetExchangePathFilter: GraphEdgeFiltering {
    func shouldVisit(edge: Edge, predecessor: Edge?) -> Bool {
        guard
            let chainIn = chainRegistry.getChainInterface(for: edge.origin.chainId),
            let assetIn = chainIn.assetInteface(for: edge.origin.assetId),
            let chainOut = chainRegistry.getChainInterface(for: edge.destination.chainId),
            let assetOut = chainOut.assetInteface(for: edge.destination.assetId) else {
            return false
        }
