...
/// is ``markProcessed(paymentId:)``, flipped once the operation reaches a terminal status; "active"
...
public protocol IncomingPaymentStoring: Sendable {
    ...
    /// The payment with this id, if any — the idempotency check for `accept`.
    func fetch(byId id: IncomingPaymentId) async throws -> IncomingPayment?
    ...
    /// Flips `processed` to `true` via a partial mapper (separate from `save` to avoid
    /// fetch-modify-save races — see CLAUDE.md). Idempotent.
    func markProcessed(paymentId: IncomingPaymentId) async throws

    /// Streams a single payment's snapshots: the current value, then every change (`nil` when absent).
    func observePayment(id: IncomingPaymentId) -> AnyAsyncSequence<IncomingPayment?>
