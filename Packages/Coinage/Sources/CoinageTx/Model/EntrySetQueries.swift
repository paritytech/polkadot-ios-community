...
/// apart.
public extension Sequence<CoinageTxEntry> {
    /// Identifiers of the inputs of every entry whose status matches.
    func inputIdentifiers(where matches: (CoinageTxStatus) -> Bool = { _ in true }) -> Set<String> {
    ...
    func outputIdentifiers(where matches: (CoinageTxStatus) -> Bool = { _ in true }) -> Set<String> {
    ...
    func assetIdentifiers(where matches: (CoinageTxStatus) -> Bool = { _ in true }) -> Set<String> {
    ...
    func mintersByOutputIdentifier() -> [String: CoinageTxEntry] {
