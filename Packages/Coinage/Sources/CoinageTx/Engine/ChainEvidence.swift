...
public struct ChainEvidence: Sendable {
    ...
    public let presenceAtFinalized: [String: ChainPresence]
    public let presenceAtBest: [String: ChainPresence]
    public let aliasAtFinalized: [String: AliasRead]
    public let aliasAtBest: [String: AliasRead]
    ...
    public init(
        finalized: BlockRef,
        best: BlockRef,
        presenceAtFinalized: [String: ChainPresence],
        presenceAtBest: [String: ChainPresence],
        aliasAtFinalized: [String: AliasRead],
        aliasAtBest: [String: AliasRead],
...
public extension ChainEvidence {
    func exists(_ key: String, atFinalized: Bool) -> Bool {
        presence(atFinalized)[key] == .present
    }

    func absent(_ key: String, atFinalized: Bool) -> Bool {
        presence(atFinalized)[key] == .absent
    }

    func isUnloaded(_ key: String, atFinalized: Bool) -> Bool {
        alias(atFinalized)[key] == .unloaded
    }

    func isNotUnloaded(_ key: String, atFinalized: Bool) -> Bool {
    ...
    func executed(_ entry: CoinageTxEntry, atFinalized: Bool) -> Bool {
        entry.outputs.contains { exists($0.identifier, atFinalized: atFinalized) }
            || entry.inputs.contains { !$0.isCoin && isUnloaded($0.identifier, atFinalized: atFinalized) }
    }

    private func presence(_ atFinalized: Bool) -> [String: ChainPresence] {
        atFinalized ? presenceAtFinalized : presenceAtBest
    }

    private func alias(_ atFinalized: Bool) -> [String: AliasRead] {
