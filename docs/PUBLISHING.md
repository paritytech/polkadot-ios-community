...
chain set is missing, it stays on the startup screen with a "waiting for
network" notice.
...
| `funding_domain` | string | DotNS label of the funding ("get some") dApp opened from the wallet. |
...
| `BRAND_LOGS_EMAIL` | Recipient of the log archive sent from Debug Settings |
