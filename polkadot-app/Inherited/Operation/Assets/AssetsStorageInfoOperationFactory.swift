...
extension AssetStorageInfoOperationFactoryProtocol {
    func createAssetBalanceExistenceOperation(
        ...
        let existenseBalanceWrapper = OperationCombiningService.compoundNonOptionalWrapper(
            operationManager: OperationManager(operationQueue: operationQueue)
        ) {
            let storageInfo = try storageInfoWrapper.targetOperation.extractNoCancellableResultData()

            return self.createAssetBalanceExistenceOperation(
                for: storageInfo,
                chainId: chainId,
                asset: asset
            )
        ...
        existenseBalanceWrapper.addDependency(operations: [storageInfoWrapper.targetOperation])

        return existenseBalanceWrapper.insertingHead(operations: storageInfoWrapper.allOperations)
    }
}
...
extension AssetStorageInfoOperationFactory: AssetStorageInfoOperationFactoryProtocol {
    ...
    private func createAssetsExistenceOperation(
        ...
        let assetsDetailsPath = AssetsPallet.detailsPath(from: extras.palletName)
        ...
        let fetchWrapper: CompoundOperationWrapper<[StorageResponse<AssetsPallet.Details>]> = requestFactory.queryItems(
            engine: connection,
            keyParams: {
                let codingFactory = try codingFactoryOperation.extractNoCancellableResultData()
                let param = try AssetsPalletSerializer.decode(
    ...
    private func createNativeAssetExistenceOperation(
        ...
        existentialDepositConstantOperation(
            path: BalancesPallet.existentialDepositPath,
            for: runtimeService
        )
    }
