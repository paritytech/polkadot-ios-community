...
protocol ConnectionPoolProtocol {
    ...
    func setConnectionEnforced(_ isEnforced: Bool, for chainId: ChainModel.Id)
...
class ConnectionPool {
    ...
    private var areConnectionsSleeping = false
    private var enforcedConnections = Set<ChainModel.Id>()
...
extension ConnectionPool: ConnectionPoolProtocol {
    ...
    func setConnectionEnforced(_ isEnforced: Bool, for chainId: ChainModel.Id) {
        ...
        if isEnforced {
            enforcedConnections.insert(chainId)
        } else {
            enforcedConnections.remove(chainId)
        }

        guard let connection = connections[chainId]?.target as? ChainConnection else {
            return
        }

        if isEnforced {
            if areConnectionsSleeping {
                connection.connect()
            }
        } else if areConnectionsSleeping {
            connection.disconnect(true)
        }
    }
}
...
extension ConnectionPool: ApplicationHandlerDelegate {
    ...
    func didReceiveDidEnterBackground(notification _: Notification) {
        ...
        areConnectionsSleeping = true

        connections.forEach { chainId, wrapper in
            guard !enforcedConnections.contains(chainId) else {
                return
            }
