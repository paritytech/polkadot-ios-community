...
class SubstrateLocalSubscriptionFactory: BaseLocalSubscriptionFactory {
    ...
    func getNoFallbackScalingElementProvider<T: Decodable & Equatable>(
        for storagePath: StorageCodingPath,
        encodableElement: some ScaleEncodable,
        chainId: ChainModel.Id
    ) throws -> AnyDataProvider<ChainStorageDecodedItem<T>> {
        let localKey = try LocalStorageKeyFactory().createFromStoragePath(
            storagePath,
    ...
    func getAccountBasedProvider<T: Equatable & Decodable>(
        for chainId: ChainModel.Id,
        accountId: AccountId,
        storagePath: StorageCodingPath,
        shouldUseFallback: Bool = true
