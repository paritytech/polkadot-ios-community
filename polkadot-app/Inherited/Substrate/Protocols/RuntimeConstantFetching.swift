...
struct RuntimeConstantCallback<T> {
    let queue: DispatchQueue
    let closure: (Result<T, Error>) -> Void
}
...
protocol RuntimeConstantFetching {
    func fetchConstant<T: LosslessStringConvertible & Equatable>(
        ...
        fallbackValue: T?,
        callback: RuntimeConstantCallback<T>
    ) -> CancellableCall
...
extension RuntimeConstantFetching {
    ...
    func fetchConstant<T: LosslessStringConvertible & Equatable>(
        ...
        fetchConstant(
            ...
            callback: .init(queue: .main, closure: closure)
        )
    }
    ...
    func fetchConstant<T: LosslessStringConvertible & Equatable>(
        ...
        execute(
            ...
            runningCallbackIn: callback.queue
        ) { result in
