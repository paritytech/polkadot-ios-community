...
enum BlockTimeEstimationServiceError: Error {
    case corruptedBlockTime
}
...
final class BlockTimeEstimationService {
    ...
    private let syncQueue = DispatchQueue(
        label: "io.block.time.\(UUID().uuidString)",
        qos: .userInitiated
    )
    ...
    private func setupInitialSnapshot(for estimatedBlockTime: EstimatedBlockTime) {
        syncQueue.async { [weak self] in
            let snapshot = Snapshot(
                blockTime: estimatedBlockTime.blockTime.millisecondsToSeconds(),
                lastBlock: nil,
    ...
    private func subscribeBlockNumber() {
        ...
        let storagePaths: [StorageCodingPath] = [SystemPallet.blockNumberPath, TimestampPallet.timestampNowPath]
    ...
    private func processBlockNumber(_ model: BlockTimeSubscriptionModel) {
        ...
        if
            ...
            let newBlockTimeElement = (currentTime - prevTime).millisecondsToSeconds()
    ...
    func createEstimatedBlockTimeOperation() -> BaseOperation<EstimatedBlockTime> {
        ClosureOperation {
            ...
            guard let info = fetchedInfo else {
                throw BlockTimeEstimationServiceError.corruptedBlockTime
            }
