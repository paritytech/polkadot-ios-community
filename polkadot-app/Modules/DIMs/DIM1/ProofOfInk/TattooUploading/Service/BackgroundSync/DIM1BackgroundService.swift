...
protocol DIM1BackgroundServiceProtocol: AnyObject {
    var delegate: DIM1BackgroundServiceDelegate? { get set }
...
protocol DIM1BackgroundServiceDelegate: AnyObject {
...
final class DIM1BackgroundService: DIM1BackgroundServiceProtocol {
    weak var delegate: DIM1BackgroundServiceDelegate?

    private let fetcher: DIM1BackgroundStateFetcher
    private let logger: LoggerProtocol

    private var isInBackground = false

    init(
        fetcher: DIM1BackgroundStateFetcher,
    ...
    // MARK: - DIM1BackgroundServiceProtocol

    func startObserving() {
        DIM1BackgroundTaskRegistrator.shared.service = self
        addAppLifecycleObservers()
    }

    func stopObserving() {
        DIM1BackgroundTaskRegistrator.shared.service = nil
...
private extension DIM1BackgroundService {
    enum TimeIntervals {
        static let taskEarliestBeginInterval = TimeInterval(60 * 60 * 2)
    }

    var stateRefreshTaskId: String {
        DIM1BackgroundTaskRegistrator.BackgroundTaskIds.stateRefresh
    }
