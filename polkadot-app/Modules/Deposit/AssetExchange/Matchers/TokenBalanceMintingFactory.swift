...
private extension TokenBalanceMintingFactory {
    ...
    private func createStatemineTokenMintWrapper(
        ...
        do {
            ...
            let assetsDetailsPath = AssetsPallet.detailsPath(from: info.palletName)
            let requestFactory = StorageRequestFactory(
                remoteFactory: StorageKeyFactory(),
                operationManager: OperationManager(operationQueue: operationQueue)
            )

            let fetchWrapper: CompoundOperationWrapper<[StorageResponse<AssetsPallet.Details>]>
            ...
            let mappingOperation = ClosureOperation<RuntimeCallCollecting> {
                ...
                .addingLast(
                    AssetsPallet.MintCall(
                        assetId: info.assetId,
                        beneficiary: .accoundId(accountId),
                        amount: amount
                    )
                    .runtimeCall(for: info.palletName ?? AssetsPallet.name)
                )
    ...
    private func createOrmlTokenMintWrapper(
        ...
        let call = OrmlPallet.SetBalanceCall(
            who: .accoundId(accountId),
    ...
    func createTokenMintingWrapper(
        ...
        do {
            guard let chainAssetModel = chainAsset as? ChainAsset else {
                throw TokenBalanceMintingFactoryError.unsupportedAsset(chainAsset)
            }

            let runtimeProvider = try chainRegistry.getRuntimeProviderOrError(for: chainAssetModel.chain.chainId)
